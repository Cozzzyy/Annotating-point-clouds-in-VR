...
export function Hud() {
    const {isHudOpen, isLabelingOpen, setIsHudOpen} = useHud();
    ...
    const scale = new THREE.Vector3(0.65, 0.65, 0.35);
    // Base offset (in meters) from the controller.
    const baseOffsetDistance = -1.8;
    ...
    const speedMultiplier = 10; // Adjust this value as needed
    ...
    useFrame(() => {
        if (leftController && leftController.object) {
            ...
            if (xButton) {
                if (xButton.state === "pressed" && !wasXPressed.current) {
                    // Toggle the HUD only once when the button transitions to pressed.
        ...
        console.log("Hud open: ", isHudOpen);
    });
